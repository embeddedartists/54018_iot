...
{
	TickType_t ttTarget = xTaskGetTickCount();
    EventBits_t event_bits;
	EventGroupHandle_t event_group = ds_subscribe();
	if (event_group == NULL) {
		PRINTF("Failed to subscribe to Data Store\n");
		vTaskDelete(NULL);
		return;
	}
	if (!ds_read(&message)) {
		PRINTF("Failed to read initial message\n");
		vTaskDelete(NULL);
		return;
	}
	...
	while (true) {
		event_bits = xEventGroupWaitBits(event_group,    /* The event group handle. */
						DS_BUTTON_CHANGED | DS_WEATHER_CHANGED,    /* The bit pattern the event group is waiting for. */
						pdTRUE,         /* The event bits will be cleared automatically. */
						pdFALSE,        /* Don't wait for both bits, either bit unblock task. */
						portMAX_DELAY); /* Block until the condition to is met. */

		if ((event_bits & (DS_WEATHER_CHANGED | DS_BUTTON_CHANGED)) > 0)
		{
			ds_read(&message);
			if ((event_bits & DS_BUTTON_CHANGED) == DS_BUTTON_CHANGED)
			{
				...
				if (xTaskGetTickCount() <= ttTarget) {
					/* Weather is updated every 5 seconds. By checking the current
					 * time too frequent updates of the display can be prevented. */
					continue;
				}
			}
		}

		PRINTF("Started updating ePaper\n");
		displayEPD(message.temperature*10, message.pressure, message.humidity*10);
		PRINTF("Finished updating ePaper\n");

		/* Next possible update is a minute from now */
		ttTarget = xTaskGetTickCount() + 60000/portTICK_PERIOD_MS;
	}
}
