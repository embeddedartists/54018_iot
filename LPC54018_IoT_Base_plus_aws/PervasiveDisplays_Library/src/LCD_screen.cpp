...
void LCD_screen::showInformation(uint16_t x0, uint16_t y0)
{
    ...
    //gText(x0, y0, "release " + String(LCD_SCREEN_RELEASE / 100) + "." + String(LCD_SCREEN_RELEASE / 10 % 10) + String(LCD_SCREEN_RELEASE % 10));
		sprintf(info_buff, "release %d.%d%d", LCD_SCREEN_RELEASE / 100, LCD_SCREEN_RELEASE / 10 % 10, LCD_SCREEN_RELEASE % 10);
		gText(x0, y0, info_buff);
    y0 += fontSizeY() + 1;
    ...
    //gText(x0, y0, "Screen:   " + WhoAmI());
	  sprintf(info_buff, "Screen:   %s", WhoAmI());
		gText(x0, y0, info_buff);
    y0 += fontSizeY() + 1;
    //gText(x0, y0, "Pixels:   " + String(screenSizeX()) + " x " + String(screenSizeY()));
	  sprintf(info_buff, "Pixels:   %d x %d", screenSizeX(), screenSizeY());
		gText(x0, y0, info_buff);
    y0 += fontSizeY() + 1;
    //gText(x0, y0, "Readable: " + String((isReadable()) ? "yes" : "no"));
	  sprintf(info_buff, "Readable: %s", isReadable() ? "yes" : "no");
		gText(x0, y0, info_buff);
    y0 += fontSizeY() + 1;
    //gText(x0, y0, "Touch:    " + String((isTouch()) ? "yes" : "no"));
		sprintf(info_buff, "Touch:    %s", isTouch() ? "yes" : "no");
		gText(x0, y0, info_buff);
    y0 += fontSizeY() + 1;
    //gText(x0, y0, "Storage:  " + String((isStorage()) ? "yes" : "no"));
		sprintf(info_buff, "Storage:  %s", isStorage() ? "yes" : "no");
		gText(x0, y0, info_buff);
    y0 += fontSizeY() + 1;
}
...
void LCD_screen::displayImage(uint16_t x0, uint16_t y0, uint16_t w, uint16_t h, uint16_t fg, uint16_t bg, const uint8_t* img)
{
  for (int y = 0; y < h; y++) {
	  for (int x = 0; x < w/8; x++) {
			uint8_t val = img[y*w/8 + x];
			if (bg != greenColour)
			{
				_setPoint(x*8+0+x0, y+y0, ((val>>7)&1)?bg:fg);
  			_setPoint(x*8+1+x0, y+y0, ((val>>6)&1)?bg:fg);
	  		_setPoint(x*8+2+x0, y+y0, ((val>>5)&1)?bg:fg);
		  	_setPoint(x*8+3+x0, y+y0, ((val>>4)&1)?bg:fg);
			  _setPoint(x*8+4+x0, y+y0, ((val>>3)&1)?bg:fg);
  			_setPoint(x*8+5+x0, y+y0, ((val>>2)&1)?bg:fg);
	  		_setPoint(x*8+6+x0, y+y0, ((val>>1)&1)?bg:fg);
		  	_setPoint(x*8+7+x0, y+y0, ((val>>0)&1)?bg:fg);
			}
			else
			{
				if (!((val>>7)&1)) _setPoint(x*8+0+x0, y+y0, fg);
  			if (!((val>>6)&1)) _setPoint(x*8+1+x0, y+y0, fg);
	  		if (!((val>>5)&1)) _setPoint(x*8+2+x0, y+y0, fg);
		  	if (!((val>>4)&1)) _setPoint(x*8+3+x0, y+y0, fg);
			  if (!((val>>3)&1)) _setPoint(x*8+4+x0, y+y0, fg);
  			if (!((val>>2)&1)) _setPoint(x*8+5+x0, y+y0, fg);
	  		if (!((val>>1)&1)) _setPoint(x*8+6+x0, y+y0, fg);
		  	if (!((val>>0)&1)) _setPoint(x*8+7+x0, y+y0, fg);			}
		}
	}
}
...
bool LCD_screen::_inValue(int16_t value, int16_t valueLow, int16_t valueHigh)
{
    ...
    {
        return ((valueLow <= value) && (value < valueHigh));
    }
    else
    {
...
{
    red   = (rgb & 0xf800/*0b1111100000000000*/) >> 11 << 3;
    green = (rgb & 0x07e0/*0b0000011111100000*/) >>  5 << 2;
    blue  = (rgb & 0x001f/*0b0000000000011111*/)       << 3;
}
uint16_t LCD_screen::halveColour(uint16_t rgb)
{
    return ((rgb & 0xf800/*0b1111100000000000*/) >> 12 << 11 | \
            (rgb & 0x07e0/*0b0000011111100000*/) >>  6 <<  5 | \
            (rgb & 0x001f/*0b0000000000011111*/) >>  1);
}
...
{
    return (uint16_t)(rgb ^ 0xffff/*0b1111111111111111*/);
}
